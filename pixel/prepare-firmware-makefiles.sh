...
setup_makefiles() {
  ...
  local bootloader_sha1=$(sha1sum "${vendor_path}/factory/bootloader-${device}-${bootloader_version,,}.img")
  if [[ "${_wifi_only}" != "true" ]]; then
    local radio_version=$(cat "${vendor_path}/android-info.txt" | grep version-baseband | cut -d = -f 2)
    local radio_sha1=$(sha1sum "${vendor_path}/factory/bootloader-${device}-${radio_version,,}.img")
  fi
