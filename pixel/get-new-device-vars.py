...
def get_aosp_branches_for_build_ids(aosp_branches, m):
    try:
...
def main():
    ...
    parser.add_argument('-t', '--tags_match', help='Android version tag to match, such as android-13.0', type=str, required=True)
    parser.add_argument('-br', '--branch_match', help='Android version branch to match, such as android13', type=str, required=True)
    ...
    all_aosp_branches = get_all_aosp_branches("{0}*".format(args.branch_match))
    build_id_aosp_branch_map = {}
    get_aosp_branches_for_build_ids(all_aosp_branches, build_id_aosp_branch_map)
    ...
    def handle_device(device, build_id, output_fn):
        ...
        aosp_branch = build_id_aosp_branch_map.get(build_id, 'unknown')
        output_fn('new_aosp_branch="{0}"'.format(aosp_branch))
