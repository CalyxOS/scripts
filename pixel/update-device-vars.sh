...
get_new_device_vars() {
  local d="${1}"
  local tmp=$(mktemp "${2}/${d}.XXXXXXXXXX")
  local dv="${3}/${d}"
  source "${dv}"
  ${4}/get-new-device-vars.py -b "${build_id}" -d "${d}"> "${tmp}"
  source "${tmp}"
...
main() {
  ...
  parallel get_new_device_vars ::: ${ds} ::: "${tmp_dir}" ::: "${vars_path}" ::: "${script_path}"
}
