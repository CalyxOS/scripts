...
help() {
  ...
  echo
  echo "Example usage:"
  echo "  $0 gen_allowlist calyx/keys/aosp-test-keys > aosp-test-keys.txt"
  echo "  m target-files-package"
  echo '  DENIED_FINGERPRINTS="$(cat aosp-test-keys.txt)" $0 target_files \'
  echo '    "$OUT/obj/PACKAGING/target_files_intermediates/*-target_files.zip"'
}
...
CHECK_MODE="${CHECK_MODE:-allowlist}"
...
handle_target_files() {
  ...
  if [ "${#apps[@]}" -gt 1 ]; then
    ...
    if [ -n "$output" ]; then
      echo >&2
      echo "TEST KEYS OR OTHER ISSUES FOUND WITH:" >&2
      printf "%s\n" "$output"
      return 1
    fi
    return $?
