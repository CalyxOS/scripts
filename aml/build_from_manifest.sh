...
commit_prebuilt_module() {
  ...
  for output_file in "${output_files[@]}"; do
    ...
    git -C "$aml_prebuilts_dir/$module" add "$filename" || return $?
  done
  git -C "$aml_prebuilts_dir/$module" commit -a -m "$commit_msg" || return $?
}

commit_prebuilt_sdk() {
  git -C "$sdk_prebuilts_dir/$sdk" add current || return $?
  git -C "$sdk_prebuilts_dir/$sdk" commit -a -m "$commit_msg" || return $?
}
