...
module_error() {
  error_m "$(module_print "$1")" "${2:-1}" || return $?
}
...
copy_output_files() {
  ...
  if [ "${#output_files[@]}" -gt 0 ]; then
    ...
    populate_applicable_output_files "$module_out_dir" || return $?
    for output_file in "${output_files[@]}"; do
      local filename="$(basename "$output_file")"
      local destfile="$module_out_dir/$filename"
      if [ -e "$destfile" ]; then
        git -C "$module_out_dir" rm -f "$filename" || rm -f "$destfile"
      fi
    done
  ...
  for output_file in "${output_files[@]}"; do
    ...
    cp -d --preserve=all "$output_file" "$module_out_dir/" || return $?
    any_copied=1
  done

  copy_extra_files || return $?
...
copy_extra_files() {
  ...
  while read -r extra_file; do
    ...
    local dst="$module_out_dir/${extra_file#*:}"  # everything after the first colon
    [ -e "$src" ] || module_error "Could not find extra file $src" $? || return $?
...
prep_and_build_module() {
  ...
  module_out_dir="$AML_PREBUILTS_DIR/$module"
  sdk_out_dir="$SDK_PREBUILTS_DIR/$sdk"

  local do_build_module=
  local do_build_sdk=
  if [ "$REPO_ONLY" == "n" ]; then
    [ "$BUILD_MODULE" == "n" ] || do_build_module=1
    ! should_build_sdk || do_build_sdk=1
  fi

  local do_copy_module_output=
  local do_copy_sdk_output=
  local do_commit_module_output=
  local do_commit_sdk_output=
  case "$COPY_OUTPUT_FILES" in
    y)
      do_copy_module_output=1
      do_copy_sdk_output=1
      ;;
    module)
      do_copy_module_output=1
      ;;
  ...
  case "$COMMIT_OUTPUT_FILES" in
    y)
      do_commit_module_output=1
      do_commit_sdk_output=1
      ;;
    module)
      do_commit_module_output=1
      ;;
  ...
  local missing=
  if [ -n "$do_build_module" ] && [ ! -d "$module_out_dir" ]; then
    module_error "Module output directory does not exist: $module_out_dir" || missing=1
  fi
  if [ -n "$do_build_sdk" ] && [ ! -d "$sdk_out_dir" ]; then
    module_error "SDK output directory does not exist: $sdk_out_dir" || missing=1
  fi
  [ -z "$missing" ] || return 1

  repo_init_and_sync || module_error "Failed to prepare for module" $? || return $?
  ...
  if [ -n "$do_build_module" ]; then
    module_print "Building"
    populate_applicable_output_files || return $?
    rename_files_to_dot_old "${output_files[@]}" || return $?
    build_module || module_error "Failed to build" $? || return $?
    if [ "$compressible_apex" == "1" ]; then
      compress_apex_and_sign || module_error "Failed to compress and sign APEX" $? || return $?
    fi
    if [ -n "$do_copy_module_output" ]; then
      copy_output_files || module_error "No output files found after building module" $? \
          || return $?
      if [ -n "$do_commit_module_output" ]; then
        commit_prebuilt_module || module_error "Failed to commit module" $? || return $?
      fi
    fi
  fi

  if [ -n "$do_build_sdk" ]; then
    populate_sdk_applicable_output_files || return $?
    rename_files_to_dot_old "${sdk_output_files[@]}" || return $?
    module_print "Building SDK"
    build_module_sdk || module_error "Failed to build module SDK" $? || return $?
    if [ -n "$do_copy_sdk_output" ]; then
      extract_sdk_output_files \
          || module_error "Failed to extract output files after building module SDK" $? \
          || return $?
      if [ -n "$do_commit_sdk_output" ]; then
...
init() {
  # Unset environment variables that may interfere with build: ANDROID_*, TARGET_*,
  # OUT, BUILD_NUMBER.
  # Keep TARGET_RELEASE for now though, as it may be needed in some cases.
  unset $(compgen -v | grep '^ANDROID_\|^TARGET_' | grep -Fxv TARGET_RELEASE)
  unset OUT BUILD_NUMBER
}
