...
is_valid_tag() {
  [[ "$tag" =~ _[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]+$ ]]
}
...
repo_init_and_sync() {
  ...
  git -C "$AML_MANIFEST_DIR" tag "$temp_tag" >/dev/null || return $?
...
prep_and_build_module() {
  local err=0
  module="$1"
  local requested_tag="${modules_to_tags[$module]:-}"
  ...
  tag="$(git -C "$aml_buildroot/$repo" name-rev --tags --name-only HEAD)"
  tag="${tag%^*}"
  if [ -z "$tag" ] || ! is_valid_tag "$tag"; then
    tag="$requested_tag"
  fi
  if [ "$tag" == "$requested_tag" ]; then
    module_print "Preparing $tag"
    commit_msg="$tag $(date +%Y-%m-%d)"
  else
    module_print "Preparing $tag ($requested_tag was requested)"
    commit_msg="$tag (requested $requested_tag) $(date +%Y-%m-%d)"
  fi
