...
repo_init_and_sync() {
  ...
  # repo wants to check out our apex_manifest repo, so we need to give it a branch.
  # If we are on a branch now, use that. Otherwise, create a temporary tag.
  local orig_branch="$(git rev-parse --abbrev-ref HEAD)"
  local switch_to="$orig_branch"
  if [ "$orig_branch" == "HEAD" ]; then
    switch_to=temp_build_for_manifest
    git -C "$apex_manifest_dir" tag -d "$switch_to" 2>/dev/null || true
    git -C "$apex_manifest_dir" tag "$switch_to" || return $?
  fi
  local repo_init_args=(-u "$apex_manifest_dir" -m "$manifest_file" -b "$switch_to")
  ...
  if [ "$orig_branch" == "HEAD" ]; then
    # Remove our temporary branch and return to the previous branch.
    git -C "$apex_manifest_dir" tag -d "$switch_to" || return $?
  fi
  return $err
}
