...
BUILD_MODULE_SDK="${BUILD_MODULE:-y}"       # - build the module SDK, if applicable
MODULE_ARCH="${MODULE_ARCH:-arm64}"         # - module arch; defaults to arm64
...
prep_and_build_module() {
  ...
  if [ "$REPO_ONLY" == "n" ]; then
    ...
    if [ -n "$sdk" ] && [ "$BUILD_MODULE_SDK" != "n" ]; then
      populate_sdk_applicable_output_files
