...
## 1. FLAGS ##
## Valid values are y, n, or sometimes force.
  ...
  for generated_file in "${output_files[@]}"; do
    case "$generated_file" in
      *.apex)
        ...
        if [ -z "$key" ]; then
          error_m "[$module] Key for $filename not found in metadata"
          return 1
        fi
        ...
        if [ ! -e "$public_key" ] || [ ! -e "$private_key" ]; then
          error_m "[$module] Key(s) not found: $public_key / $private_key"
          return 1
        fi
        module_print "Signing compressed APEX..."
