...
switch_and_sync_tag() {
  ...
  if ! workaround_for_manifest_duplicates; then
    echo "# Could not apply manifest duplicates workaround; maybe already applied" >&2
  fi
...
aml_build_module_common() {
  ...
  local do_workarounds_for_bad_tags_after=
  workarounds_for_bad_tags "$tag" || err=$?
  if [ $err -ne 0 ]; then
    do_workarounds_for_bad_tags_after=1
  fi
  ...
  if true || ! is_manifest_on_tag "$tag"; then
    ...
    if [ -n "$do_workarounds_for_bad_tags_after" ]; then
      #switch_and_sync_tag "$tag" || return $?
      workarounds_for_bad_tags "$tag" || return $?
    fi
...
#     SOONG_SDK_SNAPSHOT_USE_SRCJAR=true \
    ...
    if ! $eval_or_skip 'cp -a "$sdks_dir/mainline-sdks"/* "$aml_pkg_dir/module_sdk"'; then
      echo "# No useful output files found for module SDK $module package $pkg" >&2
      $eval_or_skip 'rmdir "$aml_pkg_dir/module_sdk" || touch "$aml_pkg_dir/module_sdk/stale"'
    else
...
EOF
      $do_or_print repo sync external/apache-velocity-engine external/owasp/java-encoder || return $?
      ;;
    aml_res_331820000)
...
EOF
        $do_or_print repo sync external/rust/crates/octets || return $?
      fi
      ;;
    aml_uwb_331910010)
      cat <<'EOF' >>"$workarounds_manifest"
...
EOF
      $do_or_print repo sync external/apache-velocity-engine external/owasp/java-encoder \
        external/rust/crates/synstructure external/rust/crates/zeroize \
        external/rust/crates/zeroize_derive || return $?
      ;;
    aml_art_331813100|aml_wif_331910020)
      cat <<'EOF' >>"$workarounds_manifest"
<project revision="refs/tags/android-u-beta-1-gpl" path="external/apache-velocity-engine" name="platform/external/apache-velocity-engine" groups="pdk" />
EOF
      $do_or_print repo sync external/apache-velocity-engine || return $?
      ;;
  esac
