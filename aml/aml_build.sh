...
main() {
  ...
  if [ ! -d "$aml_out_dir" ]; then
    echo "Could not find module output directory: $modules_out_dir" >&2
    return 1
  fi
  aml_sdk_out_dir="$target_android_dir/prebuilts/module_sdk"
  if [ ! -d "$aml_sdk_out_dir" ]; then
...
workaround_for_compressed_apex() {
  ...
  workaround_for_compressed_apex_soong_2 "$@" || return $?
...
#       SOONG_SDK_SNAPSHOT_USE_SRCJAR=true \
      ...
      if [ "$arch" == "arm64" ]; then
        module_out_dir="$aml_out_dir/$module"
      else
        module_out_dir="$aml_out_dir/$module/$arch"
      fi
      ...
      [ -d "$module_out_dir" ] || mkdir "$module_out_dir"
...
#     SOONG_SDK_SNAPSHOT_USE_SRCJAR=true \
    ...
    echo "## Copy module SDK output"
    if [ -n "$dry_run" ]; then
      printf 'cp -a %q/mainline-sdks/* %q\n' "$sdks_dir" "$module_sdk_out_dir"
      continue
    fi
    if ! $eval_or_skip 'cp -a "$sdks_dir/mainline-sdks"/* "$module_sdk_out_dir"'; then
