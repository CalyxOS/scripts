...
#     SOONG_SDK_SNAPSHOT_USE_SRCJAR=true \
    ...
    if [ -n "$dry_run" ]; then
      ...
      $eval_or_skip 'for dir in "$sdks_dir/mainline-sdks/for-${MODULE_SDK_BUILD}-build/current/$pkg"/*/; do dirname="$(basename "$dir")"; [ "$dirname" != "sdk" ] || continue; unzip "$dir"/*.zip -d "$module_sdk_out_dir/current"; done'
