...
EOF
  ...
  paths_with_workarounds+=("${projects_to_sync[@]}")
}
  ...
  while read line; do
    case "$line" in
      "error: packages/modules/OnDevicePersonalization/federatedcompute"*)
        if [ -n "${fixes_attempted[odp]:-}" ]; then
          module_print "Skipping already-tried ODP fix"
          continue
        fi
        ...
        case "$tag" in
          *_33*|*_34*)
            ...
            ;;
          *)
            module_error "Refusing to work around ancient ODP issue" 0
            break
            ;;
        esac
        workaround='<extend-project revision="refs/heads/android14-release" name="platform/packages/modules/OnDevicePersonalization" />'
        create_workaround_for_repo "$workaround" packages/modules/OnDevicePersonalization || return $?
        fixes_attempted+=([odp]=1)
        ;;
    esac
  done <<< "$errors"
...
generate_aosp_manifest() {
  ...
  for p in "${paths_with_workarounds[@]}"; do
    ...
    local workaround_comment="  <!-- Revision altered for $escaped_path to work around build errors -->"
