...
run_cmd() {
  local cmd="$1"
  shift 1
  local msg="$(printf "Running: %s %s" "$cmd" "${*@Q}")"
  module_print "$msg" >&2
  "$cmd" "$@" || return $?
}
...
EOF
  ...
  if [ "${#projects_to_sync[@]}" -gt 0 ]; then
    run_cmd repo sync -d --force-sync "${projects_to_sync[@]}" || return $?
  fi
}
...
generate_aosp_manifest() {
  ...
  local -a repo_args=(--no-pager manifest -r --suppress-upstream-revision --suppress-dest-branch)
  #module_print "Running: repo %s | sed %s\n" "${repo_args[*]@Q}" "${sed_args[*]@Q}"
  run_cmd repo "${repo_args[@]}" | \
    run_cmd sed "${sed_args[@]}" > "$manifest" || return $?
}
...
repo_init_and_sync() {
  ...
  if ! run_cmd repo init "${repo_init_args[@]}"; then
  ...
  if [ "$TRY_LOCAL_SYNC" != "n" ]; then
    ...
    run_cmd repo sync -dlj16 --force-sync || err=$?
  fi

  if [ $err -ne 0 ] || [ "$TRY_LOCAL_SYNC" == "n" ]; then
    run_cmd repo sync -dj6 --force-sync || return $?
  fi
}
