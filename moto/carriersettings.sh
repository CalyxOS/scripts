...
# carriersettings:
#
#   Handle Motorola Carrier Configuration
#
#
##############################################################################
...
readonly work_dir="${WORK_DIR:-/tmp/motorola}"

source "${vars_path}/moto"
...
readonly vendor_path="${top}/vendor/motorola/${device}"
...
setup_makefiles() {
  ...
  if [[ -z "${exists}" ]]; then
    ...
    echo "  vendor/motorola/${device}/proprietary/product/etc/apns-conf.xml:\$(TARGET_COPY_OUT_PRODUCT)/etc/apns-conf.xml" >> "${mk_path}"
    echo >> "${mk_path}"
