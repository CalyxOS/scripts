...
if ! fastboot getvar product 2>&1 | grep "^product: bonito$"; then
  echo "Factory image and device do not match. Please double check"
  exit 1
fi
fastboot flash --slot=other bootloader bootloader-bonito-b4s4-0.4-8048689.img
fastboot --set-active=other reboot-bootloader
sleep 5
fastboot flash --slot=other bootloader bootloader-bonito-b4s4-0.4-8048689.img
fastboot --set-active=other reboot-bootloader
sleep 5
fastboot flash --slot=other radio radio-bonito-g670-00145-220106-B-8048689.img
fastboot --set-active=other reboot-bootloader
sleep 5
fastboot flash --slot=other radio radio-bonito-g670-00145-220106-B-8048689.img
fastboot --set-active=other reboot-bootloader
...
fastboot --skip-reboot -w update image-bonito-ap2a.240605.024.zip
