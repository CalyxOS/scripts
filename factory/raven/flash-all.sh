...
if ! fastboot getvar product 2>&1 | grep "^product: raven$"; then
  echo "Factory image and device do not match. Please double check"
  exit 1
fi
fastboot flash --slot=other bootloader bootloader-raven-slider-15.1-12292109.img
fastboot --set-active=other reboot-bootloader
sleep 5
fastboot flash --slot=other bootloader bootloader-raven-slider-15.1-12292109.img
fastboot --set-active=other reboot-bootloader
sleep 5
fastboot flash --slot=other radio radio-raven-g5123b-143112-241107-B-12620021.img
fastboot --set-active=other reboot-bootloader
sleep 5
fastboot flash --slot=other radio radio-raven-g5123b-143112-241107-B-12620021.img
fastboot --set-active=other reboot-bootloader
...
fastboot --skip-reboot -w update image-raven-ap4a.250205.002.zip
