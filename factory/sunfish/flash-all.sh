...
if ! fastboot getvar product 2>&1 | grep "^product: sunfish$"; then
  echo "Factory image and device do not match. Please double check"
  exit 1
fi
fastboot flash --slot=other bootloader bootloader-sunfish-s5-0.5-10252351.img
fastboot --set-active=other reboot-bootloader
sleep 5
fastboot flash --slot=other bootloader bootloader-sunfish-s5-0.5-10252351.img
fastboot --set-active=other reboot-bootloader
sleep 5
fastboot flash --slot=other radio radio-sunfish-g7150-00112-230505-B-10075601.img
fastboot --set-active=other reboot-bootloader
sleep 5
fastboot flash --slot=other radio radio-sunfish-g7150-00112-230505-B-10075601.img
fastboot --set-active=other reboot-bootloader
...
fastboot --skip-reboot -w update image-sunfish-ap2a.240605.024.zip
