...
if ! fastboot getvar product 2>&1 | grep "^product: crosshatch$"; then
  echo "Factory image and device do not match. Please double check"
  exit 1
fi
fastboot flash --slot=other bootloader bootloader-crosshatch-b1c1-0.4-7617406.img
fastboot --set-active=other reboot-bootloader
sleep 5
fastboot flash --slot=other bootloader bootloader-crosshatch-b1c1-0.4-7617406.img
fastboot --set-active=other reboot-bootloader
sleep 5
fastboot flash --slot=other radio radio-crosshatch-g845-00194-210812-B-7635520.img
fastboot --set-active=other reboot-bootloader
sleep 5
fastboot flash --slot=other radio radio-crosshatch-g845-00194-210812-B-7635520.img
fastboot --set-active=other reboot-bootloader
...
fastboot --skip-reboot -w update image-crosshatch-ap2a.240605.024.zip
