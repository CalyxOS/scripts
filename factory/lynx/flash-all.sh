...
fastboot getvar product 2>&1 | grep "^product: lynx$"
if [ $? -ne 0 ]; then
  echo "Factory image and device do not match. Please double check"
  exit 1
fi
fastboot flash --slot=other bootloader bootloader-lynx-lynx-14.4-11458464.img || exit $?
fastboot --set-active=other reboot-bootloader || exit $?
sleep 5
fastboot flash --slot=other bootloader bootloader-lynx-lynx-14.4-11458464.img || exit $?
fastboot --set-active=other reboot-bootloader || exit $?
sleep 5
fastboot flash --slot=other radio radio-lynx-g5300q-231218-240202-B-11396366.img || exit $?
fastboot --set-active=other reboot-bootloader || exit $?
sleep 5
fastboot flash --slot=other radio radio-lynx-g5300q-231218-240202-B-11396366.img || exit $?
fastboot --set-active=other reboot-bootloader || exit $?
...
fastboot --skip-reboot -w update image-lynx-ap1a.240405.002.a1.zip
