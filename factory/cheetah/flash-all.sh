...
if ! fastboot getvar product 2>&1 | grep "^product: cheetah$"; then
  echo "Factory image and device do not match. Please double check"
  exit 1
fi
fastboot flash --slot=other bootloader bootloader-cheetah-cloudripper-14.5-11677884.img
fastboot --set-active=other reboot-bootloader
sleep 5
fastboot flash --slot=other bootloader bootloader-cheetah-cloudripper-14.5-11677884.img
fastboot --set-active=other reboot-bootloader
sleep 5
fastboot flash --slot=other radio radio-cheetah-g5300q-240308-240517-B-11857457.img
fastboot --set-active=other reboot-bootloader
sleep 5
fastboot flash --slot=other radio radio-cheetah-g5300q-240308-240517-B-11857457.img
fastboot --set-active=other reboot-bootloader
...
fastboot --skip-reboot -w update image-cheetah-ap2a.240605.024.zip
